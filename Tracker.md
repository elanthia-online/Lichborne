...
## v0.20.0 — Triggers wait for roundtime · a redesigned Team Login (and single login) · status toasts for your other characters · a colour per character · rule moves that can't lose a rule · a Templates tab that scales

- **Numbered v0.20.0, not v0.19.10** (Sekmeht): the working version was bumped to 0.19.10 mid-cycle and then raised to 0.20.0 for release, so every note, comment and the test plan written as "v0.19.10" was renamed. There is no v0.19.10.

- **Triggers wait for roundtime (F120, B467, B468).** Sekmeht: triggers fired their commands the instant the line appeared, so in combat the game discarded them. Each command action now carries `waitForRt` (absent = ON, so every existing trigger changed behaviour; `;` Lich commands never wait). DESIGN §14.10c.
  - **The timing fact that shaped the design:** the parser emits `roundtime` at the `<prompt>`, AFTER the turn's text. At fire time the RT for the line that fired the trigger has not arrived yet, so "is RT clear right now?" answers about the PREVIOUS turn. The queue therefore waits for the next prompt to SETTLE the turn (capped at `RT_SETTLE_CAP_MS` = 1s for a turn with no prompt), then for RT to expire, then sends (new pitfall #161).
  - `TriggerGameState` holds `rtExpires`/`ctExpires` as epoch ms, so `$rt`/`$ct` are live seconds (`secondsLeft`) rather than the value reported at the start of the roundtime.
  - **B468:** a trigger watching `rt` or `ct` never fired when roundtime ENDED, because the value was only ever reported at its start. `noteTimer` now arms a timer that reports `0` at expiry — and that end event reaches ONLY rules with a condition on that variable (`onlyGated`). Found by the bug check: sent to every watcher, an unconditioned `rt` trigger fired twice a round, which the harness showed as two attacks per round.
  - `cancelPending` on a 'Disconnected' status drops the queue, so a reconnect never sends a stale command. A `ct` gate joined the condition list.
  - **Genie import keeps Genie's send timing** (verified against Genie4 Command.cs): `#send`/`#do` wait for RT, `#put` and bare commands send at once, the queue forms' leading pause (in seconds) becomes `delayMs` — it used to import as part of the command text, so `#send 2 look` became the command `2 look`. Queue CONTROL (`#send|#do|#queue clear`, and `#queue` with no delay, which LISTS Genie's queue) is dropped instead of becoming a game command. `#do` imports with a note: its stun/web wait isn't modelled. `commandOpts` stays index-aligned with `commands` through one `pushCommand` writer.
  - Slash: `/trigger add … rt=wait|now`.
- **Trigger editor clarity (F121).** Every field has a tooltip that says something the label doesn't (UX #8), each section has a one-line hint, action types carry a description, and conditions show example values (`GATE_PLACEHOLDERS`) and operator explanations. The `$var` `<select>` — which read like a setting — became **VarMenu** ([VarMenu.tsx](src/renderer/components/VarMenu.tsx)), a `$` button opening a grouped insert menu, shared with Macros/Aliases (it replaced the private `MaVarPicker`). The Variable trigger's "watch" field suggests the watchable names from a datalist. DESIGN §14.y.
- **"Applies to" moves (B470).** Sekmeht flipped a new trigger to *All Characters*: the list went blank, and deleting the "blank" one to start over deleted a different trigger. Two causes.
  - **Identity:** `trKey` was pattern-only, and a Variable-change trigger's pattern is `''` — so EVERY variable trigger was "the same rule" as every other and as any blank text trigger, and the move removes the source when the target "already has it". Variable triggers now key on the variable they watch.
  - **Key collision ≠ duplicate:** the move treated a matching key as proof of an exact copy. It now asks `sameRuleContent` (a canonical whole-record comparison — pitfall #130's shape — with a type's defaults filled first via `triggerCompareForm`, so a rule saved before a field existed still compares equal). An identical twin merges; a different rule sharing the key REFUSES the move with a toast and loses nothing. Gating counts as content when moving INTO a character.
  - **The editor follows the rule** into its new scope and reopens it (`movedOpen` → the panels' `openRuleId`; Macros gained `openMacroId`). All six editors move the FINALIZED rule — what Save would write — and disable "Applies to" while the draft can't be saved.
- **Profile Transfer reports clashes (B471).** `dropCollisions` silently skipped any incoming rule whose key matched an existing one, even when the content differed (and, via the same `trKey`, every variable trigger after the first). It now records each as a `RuleClash` in `TargetResult.clashes`, and the modal lists them (de-duplicated, amber, bounded). The kept rule is still the destination's; the report is the change.
- **Team Login, redesigned (F122).** One panel ([TeamLoginPanel.tsx](src/renderer/components/TeamLoginPanel.tsx), `.tlr-` prefix) replaces the two-step progress card + text summary. DESIGN §13.6.4.
  - A tile per member (waiting → connecting with the live connect step → ready with health/room from the Overview digests → failed with the error, or skipped). Tiles share the width (`--tlr-cols` = min(n, 4)).
  - **Play while the rest connect.** A single click CHOOSES a ready character (the footer names it: "Play *name*"); a double-click, Enter, or the footer button plays it. The panel folds into an app-bar pill and the rest connect with `addSession(info, { activate: false })`, so nothing steals focus. First-in-team-order is the default choice.
  - **Pending tabs:** characters still to come show as dashed tabs with a spinner and an indeterminate bar.
  - **Quiet separate windows:** `createWindow({ inactive })` shows with `showInactive()` on `ready-to-show`; `session:move-window({ quiet })` now returns whether it moved.
  - Retry once the run is done; "Tab closed" for a ready character whose tab you closed; a second run can't start while one is going (`teamRunBusy`); an empty window auto-expands the panel.
- **B472 — a team login could hang forever on one character.** `SGEConnection`'s read loop checked its deadline only when DATA ARRIVED, so a login server that went silent mid-handshake left `login()` pending indefinitely: no tab, no error, and the launcher showing the character as connected because its account slot was held (Sekmeht's second character). `waitFor(take, what)` now runs a real timer and settles on close/error; `readBytes`/`readRaw` sit on it. A mutation run against the old code reproduced the hang in the harness (pitfall #162).
- **Character status toasts + the Toast action (F123).** DESIGN §37.6.
  - Main sends `CHARACTER_NOTICE` (ready / dropped / reconnected) to the FOCUSED app window (`focusedAppWindow`), since the point is to tell you about a character you are NOT looking at. A drop is only a drop if the session had connected and didn't close cleanly; a connect within `RECONNECT_WINDOW_MS` of a drop reads as "reconnected". Your own foreground connects don't toast at you.
  - Clicking a notice goes to that character, across windows (`FOCUS_CHARACTER` → owner window `SELECT_CHARACTER`).
  - App-wide setting `SharedProfile.characterNotices` (default on; Settings + `/notices on|off`).
  - The trigger **Toast** action routes through main too (`ROUTE_TOAST` → the focused window), so a trigger on a background character's window still reaches you.
  - Toasts gained a `warning` kind, an `onClick`, and keyboard access when clickable (pitfall #142).
- **Live values in the `$` menu, and toasts that merge (from the UX review, Sekmeht picked #4 and #6).**
  - **VarMenu shows each variable's current value** (`LiveVarsContext`, provided by GameWindow around the Automations dialog: `buildMacroVars()` plus the clock values). Re-read once a second while open, so `$rt` visibly counts down. Values sit in the ROW, never a `title` (pitfall #145); match-only variables show nothing; own-key lookup (`hasOwnProperty`), not `in`.
  - **Toasts merge by `key`**: a toast arriving while one with the same key is on screen updates it in place (same position, no re-entry animation, timer restarted) via its `merge(prev)`. Character notices key per kind, so a network blip that drops four characters is one toast: two named in the title, more counted, a clickable badge each. Kinds never mix.
  - **People badges**: `ToastOptions.people` render as monograms coloured by `utils/nameColor.ts` — the Living Tableau's avatar recipe, lifted out so the two can't drift (the Tableau now imports it; output unchanged). The trigger Toast action carries its character's badge (`RoutedToast.character`).
  - **Hover or focus holds a toast open**; letting go returns the time it had left, never less than 2.5s — so reaching for a badge can't lose it, and holding never shortens one.
  - Harness: notices 13 → 30.
- **Character colours (F124, Sekmeht).** An optional colour per character, picked in the launcher's Edit Profile with the shared ColorField (linked named colours included), used wherever that character is drawn as a person or has a focus border. DESIGN §13.6.5.
  - **Stored app-wide** in `SharedProfile.characterColors`, keyed by `account::character` (the characterId minus its shard — see the bug check below) — Sekmeht's call: shared because every window must read it synchronously and a change must reach every window at once; keyed by id rather than name because same-named characters on different accounts already collide in profiles. [characterColors.ts](src/renderer/characterColors.ts): coerce, a cached `useSyncExternalStore` snapshot (pitfall #129), a module-level `storage` listener so the cache can't go stale with nothing subscribed, lookup by id then name.
  - **Surfaces:** toast badges; the character tab (active tint + hairline, a faint hairline when inactive — border only, so two coloured tabs never both look selected); the Overview's selection ring (ring only, per Sekmeht — no card tint); Team Login tile badge + chosen-tile ring; the launcher card (badge + hairline); the Living Tableau (for YOUR characters the character colour beats a contact template — Sekmeht had filed himself as a contact on a purple template and was purple there, teal everywhere else; other people keep their contact colour). Each surface sets `--char-color` inline and its CSS reads `var(--char-color, var(--accent))`, so no colour = byte-identical to before.
  - **Readable on any colour:** `monogramStyle` picks black or white initials by the fill (`readableTextOn`), since a player can pick anything.
  - **ColorField popovers now open above high-tier dialogs** — `aboveDialogs` adds `ui-menu--top-tier` (z 1700), because Edit Profile's backdrop is 1600 and the 450-tier menu would have opened underneath it (pitfall #118).
  - No slash command by decision; not a Transfer category (identity, like guild and notes). Harness: notices 30 → 42.
- **Team Login — the Play button moved the panel (Sekmeht).** Its width followed the chosen name ("Play Agan" / "Play Sekmeht"), which re-wrapped the hint beside it and changed the panel's height as you clicked between tiles. The button now stacks every member's label in one grid cell and shows only the chosen one (`ui-btn--stable`, a new ui.css modifier — the recipe Edit Profile's Save already used privately). Pitfall #103. Render harness 34 → 35.
- **B475 — Contacts → Templates was unusable with many templates** (a tester, via Sekmeht). The list was a flex column of `overflow: hidden` cards, and such flex items may shrink below their content (pitfall #163), so ~30 templates squashed to slivers with the open editor's Save cut off. The tab was reworked to be the Contacts tab's layout, class for class — sidebar (+ New, search with Esc/↓, scrolling list with ↑/↓ and row ✕) beside a detail pane that scrolls the editor — which removes the accordion, its scroll-into-view effect (B434) and ~120 lines of card CSS. The name field no longer autofocuses on every open (it would have stolen focus from arrowing through the list); only + New template focuses it.
- **The login panel no longer jumps as the step text wraps (Sekmeht).** `.tlr-tile-step` was clamped to two lines but not held AT two, so a one-line step followed by a two-line one resized the tile and the panel on every step. It now reserves two lines (`min-height` in its own em, with its own line-height — pitfall #116) and uses tabular numbers for the ticking seconds. Pitfall #103. Applies to team tiles too.
- **The team row's Connect button now matches the character card's (Sekmeht).** It carries `launcher-card-connect` itself, so the two can't drift apart (pitfall #113); `launcher-team-connect` only places it in the row and dims it when everyone is already on. Reads "Connect 2 →".
- **One character's login wears the Team Login look (Sekmeht).** `SoloConnectPanel`: the team chrome with one tile (live step, stall counter, bar), "Logging in", Cancel/Esc. It only draws — the `pendingConnect` flow (grace window, Cancel, conflicts, errors) is untouched. Replaced the spinner card; the `ConnectStep` leaf and `.launcher-connecting-step` went with it.
- **Bug check of this round (three review agents).** All in unshipped code, so no B-numbers (the pitfall #108 precedent).
  - *Toasts:* hover and focus held a toast through ONE flag, so the mouse leaving re-armed a toast whose chip still had focus (now separate holds); a full stack could drop the toast being held; a chip swallowed every key including Esc from the dialog underneath; chips were keyed by name (two shards → duplicate keys); a "disconnected" toast outlived the reconnect (new `withdrawNotice`); clicking one chip threw the others away (now `withoutPerson`); a repeated notice moved its chip to the end.
  - *Character colours:* keyed by the full characterId, so the launcher's DR ↔ Test toggle made the colour vanish (now `account::character`); Enter saved the raw typed text (`ember`) not the resolved link; an unpaintable value was saved and broke every surface's `color-mix()`; the colour was written before the profile save, so a failed save half-committed it; the native picker opened on black (an hsl swatch — new `nameColorHex`); the Tableau's own figure looked up by name (now `ExperienceProps.characterId`), and its avatars kept white ink on a pale picked colour (now `monogramStyle`).
  - *Templates:* Save/Close dropped the selection and stranded focus (Save now keeps it selected, Close refocuses the row); a second native ✕ in the search (`type="search"` removed); the editor could overflow sideways beside a wide sidebar (`minmax(min(240px, 100%), 1fr)`); an unclosed CSS comment; template rows showed the tag larger than contact rows; the use-count wording lived in two places.
  - *`$` menu:* placed before its values arrived, so near the left edge it could open off screen (values now read in the opening render); its width could change on the 1s refresh (fixed width while live).
  - `ui-btn--stable` replaced Edit Profile's and Attach's private copy of the same recipe.
  - Left alone: monogram ink doesn't recompute when a linked named colour changes lightness until the next render (low); and a pre-existing race where a window opened within the 2.5s shared-save debounce imports a stale `_shared.yaml` (affects every shared field, not new).
  - Harnesses: notices 42 → 63, team render 35 → 39.
- **B473 — a trigger Echo to the Game window never displayed.** `echoToStream` wrote every echo to `streamLines[key]`, and nothing renders `streamLines.main` — nor a stream with no open panel. Main (and any unwatched stream) now goes to `lines`: pinned via `appendTrimmed`, scrolled-up counted into the new-lines badge. Deferred one tick so the echo lands AFTER the line that fired it. Known gap: these echoes aren't written to the Session Log.
- **B474 —** an unknown action type (hand-edited profile, newer build) crashed the Triggers list at `ACTION_LABELS[a.type].split`. Now falls back.
- **The bug check** (four review agents plus harnesses) also fixed, in unshipped code: VarMenu's horizontal clamp and scroll-close (only for scroll containers holding its button — the game text scrolls constantly); light-theme contrast on the tiles; the clash box on `--color-warning` rather than the error colour; the Team panel composed from the `ui-modal-*` skeleton; the pill's specificity; reduced-motion and epilepsy-safe on every spinner; the `;` command's checkbox showing unticked.
- **Deliberately not fixed:** a separate window last saved MAXIMIZED may still take focus on a quiet open (skipping the maximize would lose the saved state — needs a Windows test); within one trigger a non-waiting command can send before an earlier waiting one; commands queued behind roundtime are lost if the character is moved to another window mid-queue; a turn with no RT can release the next chained command a prompt early (the settle is a heuristic); a trigger Toast is skipped while Lichborne itself isn't focused (by design — Notify is for that).
- **Harnesses (machine-local):** `tmp-trigger-harness` (run 34 — the real `useTriggerEngine` on a virtual clock; genie 17; identity 22; transfer 5) and `tmp-team-harness` (render 34 — SSR of the real panel; sge 8; notices 13).
